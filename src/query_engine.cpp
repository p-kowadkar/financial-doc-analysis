...
public:
    bool loadIndex(const std::string& indexPath) {
        ...
        // For this simplified version, we load from directory
    ...
    void executeQuery(const std::string& query, const std::string& outputFile, int topK, bool jsonFormat) {
        ...
        if (jsonFormat) {
            writeJSONResults(outFile, query, results);
        } else {
            writeMarkdownResults(outFile, query, results);
        }
...
private:
    void writeMarkdownResults(std::ofstream& file, const std::string& query, 
                             const std::vector<std::pair<std::string, double>>& results) {
        file << "# Query Results: " << query << "\n\n";
        file << "## Top Relevant Documents\n\n";
        
        for (size_t i = 0; i < results.size(); ++i) {
            file << "### " << (i + 1) << ". " << results[i].first << "\n";
            file << "Similarity Score: " << std::fixed << std::setprecision(4) << results[i].second << "\n\n";
    ...
    void writeJSONResults(std::ofstream& file, const std::string& query,
                         const std::vector<std::pair<std::string, double>>& results) {
        file << "{\n";
        file << "  \"query\": \"" << query << "\",\n";
        file << "  \"results\": [\n";
        ...
        for (size_t i = 0; i < results.size(); ++i) {
            file << "    {\n";
            file << "      \"rank\": " << (i + 1) << ",\n";
            file << "      \"document_id\": \"" << results[i].first << "\",\n";
            file << "      \"similarity_score\": " << std::fixed << std::setprecision(4) << results[i].second << ",\n";
...
int main(int argc, char* argv[]) {
    if (argc < 5) {
        std::cout << "Query Engine - Windows Compatible" << std::endl;
        std::cout << "Usage: " << argv[0] << " <index_file> <query> <output_file> <top_k> [--json]" << std::endl;
    ...
    bool jsonFormat = (argc > 5 && std::string(argv[5]) == "--json");

    QueryEngine engine;
    
    if (engine.loadIndex(indexFile)) {
        engine.executeQuery(query, outputFile, topK, jsonFormat);
    } else {
